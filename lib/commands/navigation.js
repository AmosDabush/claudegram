...
const { stopInteractiveSession, startInteractiveSession } = require('./claude');
...
function register(bot, isAuthorized) {
  ...
  bot.onText(/\/add\s+(\S+)\s+(.+)/, (msg, match) => {
    ...
    if (projectPath.startsWith('~')) {
      projectPath = projectPath.replace('~', platform.HOME);
    }
  ...
  bot.onText(/\/cd\s+(.+)/, (msg, match) => {
    ...
    if (newPath.startsWith('~')) {
      newPath = newPath.replace('~', platform.HOME);
    }
  ...
  bot.onText(/\/browse(?:\s+(.+))?/, (msg, match) => {
    ...
    if (browsePath.startsWith('~')) {
      browsePath = browsePath.replace('~', platform.HOME);
    }
...
function handleCallback(bot, query, userState) {
  ...
  if (data.startsWith('proj:')) {
    ...
    if (projectPath) {
      ...
      // Auto-start new interactive session in the selected project
      if (userState.interactiveMode !== false) {
        startInteractiveSession(userState, chatId, bot);
      }
      return true;
    }
  }
  ...
  if (data.startsWith('bs:')) {
    ...
    // Auto-start new interactive session in the selected path
    if (userState.interactiveMode !== false) {
      startInteractiveSession(userState, chatId, bot);
    }
    return true;
