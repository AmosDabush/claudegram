...
async function ensureServer() {
  ...
  try {
    // sessions-ui is a separate macOS project with a bash launcher. Where it is
    // not installed (any Windows box), there is nothing to start — say so by
    // returning false rather than spawning a shell that cannot run it.
    if (!fs.existsSync(START_SCRIPT)) return false;
