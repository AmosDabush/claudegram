...
/**
 * This whole module is macOS-only.
 *
 * Attaching injects a message into a Claude process that is already running,
 * over the unix socket that process opens at /tmp/cc-socks/<pid>.sock. Claude
 * on Windows opens no such socket and no named-pipe equivalent, so there is
 * nothing to inject into — every entry point below no-ops there and the chat
 * stays on the resume pipe, which is fully supported on both platforms.
 *
 * If per-session IPC ever lands on Windows (the session registry already
 * carries a "peerProtocol" field), this is the one flag to flip.
 */
const SUPPORTED = platform.IS_MAC;
...
function toggleRow(ctx) {
  if (!SUPPORTED) return [];      // no row at all, rather than a dead button
...
function sectionView(chatId) {
  if (!SUPPORTED) {
    return {
      text: '🔗 *Remote Ses*\n\nזמין רק על המאק — Claude ב-Windows לא פותח סוקט לכל סשן.\nכאן הכל עובר דרך *Resume Sessions*.',
      keyboard: [[{ text: '📚 Resume Sessions', callback_data: 'sessions' }]]
    };
  }
  const pid = currentTarget(chatId);
  const on = getMode() === 'attach';
...
module.exports = { register, handleCallback, maybeRoute, attached, getMode, setMode,
                   ...
                   // Every entry point here is a no-op off macOS. Exported so the menus can
                   // leave the button out rather than offer one that does nothing.
                   SUPPORTED };
