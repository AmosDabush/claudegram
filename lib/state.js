...
/**
 * Claude scopes its memory (~/.claude/projects/<cwd-key>/memory) by working directory,
 * not by session, so two chats both sitting in HOME share one memory store and overwrite
 * each other's facts. Give each chat — and each forum topic, which has its own key — a
 * workspace of its own so parallel conversations stay independent until the user
 * explicitly picks a shared project. Set CLAUDEGRAM_SHARED_HOME=1 for the old behaviour.
 */
function defaultWorkspaceFor(chatId) {
  if (process.env.CLAUDEGRAM_SHARED_HOME === '1' || chatId == null) {
    return projects.home || platform.HOME;
  }
  const slug = String(chatId).replace(/[^\w-]/g, '_');
  const dir = path.join(platform.HOME, 'claudegram-chats', slug);
  try {
    fs.mkdirSync(dir, { recursive: true });
  } catch (e) {
    console.log(`⚠️ Could not create workspace for ${chatId}: ${e.message}`);
    return projects.home || platform.HOME;
  }
  return dir;
}
 ...
 */
function getDefaultUserState(chatId) {
  return {
    currentProject: 'home',
    currentPath: defaultWorkspaceFor(chatId),
    isProcessing: false,
    ...
    // Voice notes in. Off by default because it is the one feature with dependencies
    // outside npm — a python package and a model download — and nobody should have to
    // acquire those to install a Telegram bridge for Claude. Turning it on checks the
    // host first and says what is missing.
    sttMode: 'off',
...
function getUserState(chatId) {
  if (!userStates.has(chatId)) {
    userStates.set(chatId, getDefaultUserState(chatId));
  }
  ...
  try {
    if (fs.existsSync(FILES.userState)) {
      ...
      for (const [chatIdStr, state] of Object.entries(data.users || {})) {
        const chatId = toChatKey(chatIdStr);
        const defaults = getDefaultUserState(chatId);
        // States saved before per-chat workspaces all point at HOME, which is the shared
        // store this change exists to get away from. Move them, but only while they are
        // still on the default project — an explicit /cd is the user's choice and stays.
        const legacyHome = state.currentProject === 'home' &&
          (state.currentPath === platform.HOME || state.currentPath === projects.home);
        // Merge with defaults to handle new fields
        const mergedState = {
          ...defaults,
          ...state,
          ...(legacyHome ? { currentPath: defaults.currentPath } : {}),
          voiceSettings: {
  ...
  try {
    ...
    for (const [chatId, state] of userStates) {
      ...
      const {
        ...
        turnWatchdog,          // interval handle — never serialisable
        turnInFlightSince,
        lastProgressAt,
