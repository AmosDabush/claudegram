...
// A QA run boots the real bot with synthetic updates, so it must not be able to write over
// the live sessions, user state or pid file. Moving the whole data directory is enough:
// every one of those paths is built from it.
const DATA_DIR = path.join(BOT_DIR, process.env.CLAUDEGRAM_QA === '1' ? 'data-qa' : 'data');
const PIPER_VOICES_DIR = path.join(platform.HOME, '.local/share/piper-voices');

// Directory holding the claude executable, prepended to PATH for children.
// .env still wins; otherwise resolved per platform (~/.local/bin on macOS).
const CLAUDE_BIN_PATH = process.env.CLAUDE_BIN_PATH || platform.claudeBinDir();

// Exit code bot.js uses to ask wrapper.js for a restart. It has to be distinct from a
// crash (retried, and it spends the retry budget) and from a clean exit (/close, which
// must stay down), so the wrapper can tell the three apart from the exit code alone.
...
// Default projects - resolved from the real home dir on both platforms.
